library krpcid.test;

import 'package:unittest/unittest.dart' as unit;
import 'package:hetimatorrent/hetimatorrent.dart';
import 'package:hetimacore/hetimacore.dart';
import 'dart:typed_data' as type;
import 'dart:convert' as convert;

void main() {
  unit.group('A group of tests', () {
    unit.test("> >= < <=", () {
      KId idA = new KId(new List.filled(20, 0));
      KId idB = new KId(new List.filled(20, 1));
      unit.expect(true, idB > idA);
      unit.expect(true, idB >= idA);
      unit.expect(false, idB < idA);
      unit.expect(false, idB <= idA);
      //
      unit.expect(false, idB > idB);
      unit.expect(true, idB >= idB);
      unit.expect(false, idB < idB);
      unit.expect(true, idB <= idB);
    });

    unit.test("xor", () {
      KId idA = new KId(new List.filled(20, 255));
      KId idB = new KId(new List.filled(20, 1));
      KId idC = new KId(new List.filled(20, 254));

      unit.expect(true, idC == idB.xor(idA));
    });

    unit.test("rooting table index", () {
      unit.expect(0, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00])));
      unit.expect(1, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01])));
      unit.expect(2, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x02])));
      unit.expect(2, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x03])));
      unit.expect(3, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x04])));
      unit.expect(3, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x05])));
      unit.expect(3, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x06])));
      unit.expect(3, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x07])));
      unit.expect(4, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x08])));
      unit.expect(4, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x08])));
      unit.expect(4, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x08])));
      unit.expect(5, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x10])));
      unit.expect(6, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x20])));
      unit.expect(7, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x40])));
      unit.expect(8, KRootingTable.getRootingTabkeIndex(new KId([
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x80])));
      unit.expect(160, KRootingTable.getRootingTabkeIndex(new KId([
        0x80,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
        0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00])));
      unit.expect(160, KRootingTable.getRootingTabkeIndex(new KId([
        0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
        0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff])));
    });

    //
    //
    unit.test("xor", () {
      KId idA = new KId(new List.filled(20, 0));
      KId idB = new KId(new List.filled(20, 0xff));
      unit.expect(160, KRootingTable.getRootingTabkeIndex(idA.xor(idB)));
      
      KId idC = new KId(new List.filled(20, 0^0x3));
      KId idD = new KId(new List.filled(20, 0xff^0x3));
      unit.expect(160, KRootingTable.getRootingTabkeIndex(idC.xor(idD)));
      
      KId idE = new KId(new List.filled(20, 0^0xf3));
      KId idF = new KId(new List.filled(20, 0xff^0xf3));
      unit.expect(160, KRootingTable.getRootingTabkeIndex(idE.xor(idF)));
    });
    //retrievePath(int index)
  });
}
